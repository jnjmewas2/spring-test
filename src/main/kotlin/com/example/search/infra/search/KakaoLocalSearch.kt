...
class KakaoLocalSearch(
    private val searchApiProperties: KakaoLocalSearchProperties,
    ...
    override fun search(keyword: String): CompletableFuture<List<LocalPlace>> {
        val result = try {
            ...
            header.set("Authorization", "KakaoAK " + searchApiProperties.apiKey)

            val requestEntity = HttpEntity<Map<String, Any>>(header)
            val url = searchApiProperties.apiUrl
            val uri : UriComponents
                    = UriComponentsBuilder.fromHttpUrl(url).queryParam("query", keyword).queryParam("size", COUNT).build()
            ...
            val bodyStr = mapper.writeValueAsString(responseEntity.body)

            val data = mapper.readValue(bodyStr, KakaoLocalSearchDto::class.java)
            data.documents.map { LocalPlace(it) }.toList()
